"""`ReviewOutput` — the Pydantic mirror of `eval/schema.json`, ch3's copy.

Copied in shape from `ch2_adaptation/schema.py` rather than imported: chapters communicate
only through files (`planning/03-system-design.md §1.3`), and `ch3_operation` must stay
importable with only the `ch3` extra. Both sides bind to the same shared contract,
`eval/schema.json`, instead of to each other -- the drift guard below is what stops this file
and the JSON Schema from silently diverging: if either changes without the other, import
fails loudly (NFR-20), so a schema change breaks the app at startup, not in production.
"""
...
def _assert_matches_eval_schema(schema: dict[str, Any] | None = None) -> None:
    ...
    A naive `model_json_schema() == json.load(...)` does not work: Pydantic emits a `title`
    key per property and no top-level `$schema`. So only the keys that actually constrain
    scoring are compared -- the same set `eval/tests/test_schema.py` pins, and the same set
    `ch2_adaptation/schema.py` compares.
    """
    ...
    if actual != expected:
        raise SchemaDriftError(
            ...
            "Update whichever one is stale before trusting any response validated against it."
        )
