...
AC-5 is satisfied: the full baseline run happened by hand (outside any agent session, as
designed — the GPU-budget hook blocks `--config .../full.yaml` in-session) and
`eval/results/baselines.json` is committed (`b068e3c`) with both `base_model` and
`frontier_api` entries, `n_samples: 24` for each. Base model (zero-shot): 0.00
schema-validity — every response is markdown-fenced and the harness's no-repair rule
correctly refuses to strip fences, so this is a real finding, not a defect. Frontier API,
3-shot: 1.00 schema-validity, 0.917 bug-catch (22/24), $0.00 real spend.

**Deviation from this spec's own Clarifications section, recorded rather than silently
applied:** the frontier model actually used is `gemini-3.5-flash-lite`, not the
`gemini-2.5-flash` named above. The first real run against `gemini-2.5-flash` hit a live
20-requests/day free-tier quota (fewer than the 24 needed for one run); the next attempt,
against `gemini-2.5-flash-lite`, hit a live 404 — that model is retired for this account,
with the API itself naming `gemini-3.5-flash-lite` as the successor. Full diagnosis across
three attempts is in `progress_report.md`'s Action Plan item 4 entries. Per this project's
own norm of recording a reversed decision in a new entry rather than rewriting the
original, the "Frontier model = Gemini 2.5 Flash" clarification above is left as written —
it was the correct decision with the information available at the time this spec was
planned, and this section is the record of why execution diverged from it.

C2/C3 may now begin in earnest — per FR-15, `baselines.json` exists and is committed
before C4 moves to `building`.
