...
| | |
|---|---|
| **State** | done (frozen 2026-09-16, `b216df7`; licences added `094d2c9`) |
| **Depends on** | C1 |
...
- **(2026-09-16) Mined-record labels come from the maintainer fix commits, not the frontier
  API.** Reverses the first clarification above, by the user's decision. Gemini's `label-mined`
  output put 3 of 10 `line` values outside the scorer's ±2 tolerance and got the issue wrong on
  5, while Gemini is also the frontier baseline being scored. All 10 relabeled from the fix
  commits (`label_source: maintainer-fix-commit`); see progress_report.md.
...
**Done (2026-09-16).** Task 4 completed with the `gh` CLI instead of the MCP server: 10 pre-fix
methods from Apache-2.0 `spring-projects` bug-fix commits. Labels were taken from the maintainer
fixes (see the amendment above), `freeze` produced 40 records with 0 dedup removals, and the user
moved the files into the frozen directory and committed them in `b216df7`. Checked: `load_holdout`
returns 40 = `manifest.n_total` (AC-1/AC-6); the manifest has every field listed in AC-2 plus
sources (AC-2); n_mined = 10 (AC-3); the dedup
report is committed next to the set (AC-4, and C3 dedups against `eval/frozen_hashes.txt`);
AC-5 was re-run against the hook after the freeze and all four write/read cases behaved as expected.
**Open gap, AC-7:** the licence is recorded on every mined record in `holdout.jsonl`
(`license: Apache-2.0`), but `build_manifest` writes no licence field, so the manifest does not
note it as AC-7 requires. An earlier draft of this note wrongly said it did. Fix: add a
`licenses` map to the committed manifest by hand. This changes metadata only, no record, and
nothing has been scored against the set yet.

**AC-7 closed (2026-09-17, `094d2c9`):** the manifest now has a `licenses` map (all 8 source repos
Apache-2.0). Verified that `holdout.jsonl`, `dedup_report.md` and `eval/frozen_hashes.txt` are
unchanged since `b216df7`: the edit touched metadata only.
