...
| **W&B run** | [`5wwfz38t`](https://wandb.ai/rahulyk09-iit-bombay/ch2-adaptation/runs/5wwfz38t) |
...
- **AC-3's floor was measured, not assumed, and turned out to need a real fix.** The `qlora-recipe`
  ...
  margin.
- **`max_steps` changed from 1000 to 125 after the first real GPU runs (2026-09-17, PARAM Rudra,
  A100 80GB).** The 1000-step recipe was set before C3 fixed the training set at 203 records; at
  batch 4 that is ~20 epochs. Measured with the full recipe unchanged apart from `max_steps`
  (one run each, seed 42, validation loss on `data/val.jsonl`, 23 records):
  20 → 0.917, **125 → 0.679**, 250 → 0.707, 500 → 1.160, 1000 → 1.586 (train loss at 1000:
  0.151 mean, 0.006 at the last step). The selection rule, lowest validation loss, was fixed
  before the 125/500 runs. The holdout played no part in the choice. 125 vs 250 is within noise
  on 23 records, but 500 and 1000 are clearly overfit. Caveat: `eval_steps = max_steps`, so each
  point is a separate run with its own LR decay, not a checkpoint of one run.
...
AC-2 met: the full config (`max_steps: 125`, commit `ab42114`) was launched manually on PARAM Rudra as
SLURM job 401897 (A100 80GB, 4-bit NF4). `eval_loss` 0.679 on `val.jsonl`, `train_loss` 0.6471,
2.45 epochs, 27.6 s of training. The adapter was saved to `outputs/adapter/`, then downloaded to the
laptop; sha256 of `adapter_model.safetensors` = `ace89142…5944a9cc7a`, matching the server.
AC-3: 0.4880% trainable. AC-5: W&B run `5wwfz38t` (logged offline on the node, synced from the login
node). The same seed and config reproduced sweep job 401851's losses exactly.

Published (private) to HF Hub as `rkg209/qwen2.5-coder-1.5b-java-review-qlora`, revision
`f8b1cf764d630fcad9a197750624e66b0266fd50`; the Hub's LFS sha256 of `adapter_model.safetensors` matches the server copy.
Also verified: `PeftModel.from_pretrained` loads the adapter (224 LoRA tensors, all lora_B non-zero).
