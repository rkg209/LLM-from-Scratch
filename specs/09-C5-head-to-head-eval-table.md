...
| | |
|---|---|
| **State** | done (2026-09-18 — both runs complete, table published) |
| **Depends on** | C4 |
| **Requirements** | FR-19, NFR-7, BG-2 |
| **W&B run** | [`5wwfz38t`](https://wandb.ai/rahulyk09-iit-bombay/ch2-adaptation/runs/5wwfz38t) (the C4 adapter under test; `evaluate.py` itself logs nothing to W&B) |
...
## Result (2026-09-18) — every acceptance criterion checked

| System | Schema-validity | Bug-catch | n |
|---|---|---|---|
| Fine-tuned (QLoRA, Qwen2.5-Coder-1.5B) | 0.97 (39/40) | 0.80 (32/40) | 40 |
| Base model (zero-shot) | 0.00 (0/40) | 0.00 (0/40) | 40 |
| Frontier API (3-shot) | 1.00 (40/40) | 0.82 (33/40) | 40 |

Runs: `baseline_holdout.yaml` on the laptop (base fp32 CPU + `gemini-3.5-flash-lite`, $0.00, free
tier) → `eval/results/baselines_holdout.json`; `eval_full.yaml` as PARAM Rudra SLURM job
**401990** on one A100 80GB (4-bit NF4 base + `outputs/adapter` from C4 job 401897) →
`eval/results/finetuned.json`, then re-run as job **402095** once `evaluate.py` recorded its input
hashes. The committed artifact is 402095's (sha256 `6a4b5c9b…`): same metrics, and **all 40
`raw_output` strings byte-identical to 401990's**, so the published number is reproducible across
independent jobs, not merely repeatable in aggregate. It carries `holdout_sha256` `9413aef8…`,
`schema_sha256` `8ec46637…` and `adapter_sha256` `ace89142…` next to the numbers they produced.

- **AC-1/AC-2/AC-6** — all three systems scored on the identical frozen holdout (sha256
  `9413aef8…`, printed into job 401990's log) by `eval/harness.py`, `n = 40` each, nothing
  excluded; the one invalid output counts as both invalid and uncaught.
- **AC-3** — `finetuned.json` + `baselines_holdout.json` written; table published between the
  README's `EVAL_TABLE` markers.
- **AC-4** — `per_sample` retained in both files; every aggregate here was re-derived from it.
- **AC-5** — the adapter's W&B run is recorded above and in `specs/STATUS.md`, and the
  "reproduces the table from the saved artifacts" half is now a code path,
  `ch2_adaptation/publish_table.py` (`configs/publish_table.yaml`, 6 tests), verified to
  reproduce the published README table byte-for-byte from the two committed JSON files. It
  refuses to publish if their holdout/schema hashes disagree.
- **AC-7** — the fine-tune **lost both metrics** by one record each; stated in the README in
  words, not only in cells, and analysed per-record in `docs/finetune-vs-prompting.md`.
- **AC-8** — `schema_sha256` `8ec46637…` identical across `baselines.json` (C1),
  `baselines_holdout.json` and job 401990's checksum line.

**What the result contradicts.** This spec predicted an outright schema-validity win. The
fine-tune instead reached 0.97, losing to the frontier API's constrained JSON decoding. Its
single failure is unescaped double quotes from an inlined Java snippet (`result += name + ", ";`)
inside `suggested_fix` — not the markdown fence that costs the base model all 40. The prediction
was directionally right about the mechanism (0.00 → 0.97 on identical weights) and wrong about
the ceiling; the remaining gap is a decoding-constraint problem, not a training-steps problem.
Left as a finding, not fixed — tuning against the holdout is precisely what this spec forbids.

**Historical note — the pre-run text below is kept unedited**, per the project's rule that a log
which has been cleaned up has stopped being evidence:
