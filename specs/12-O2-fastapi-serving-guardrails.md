...
See `.claude/plans/Operation.md` §O2. Summary: `api/schema.py` (`ReviewOutput` mirror + import-time drift guard against `eval/schema.json`), `api/validator.py` (`validate_output`: exactly two parse attempts, `OutputParseError`/`OutputSchemaError`), `api/errors.py` (`ErrorCode` registry, envelope, exception handlers — `RequestValidationError`→400, parse/schema failures→422, backend exception→500 `INFERENCE_ERROR`, unloaded backend→503 `MODEL_NOT_READY`), `api/middleware.py` (`RequestIdMiddleware`, `ContentSizeLimitMiddleware`). `main.py` rewired to `/v1/review` + deprecated `/review` alias. `ServeConfig` gained `max_request_bytes`/`api_version`. Done and tested (test_validator.py, test_errors.py, rewritten test_api.py — 45 tests) and verified against a real running `ch3_operation.serve` process (health, a 422 on `tiny.gguf`, a 413 on an oversized body).
