...
See `.claude/plans/Operation.md` §O3. Summary: `api/metrics.py` (`PrometheusMetrics`, own `CollectorRegistry` per instance, histogram/counter/gauge names locked in `planning/05-api-design.md §6`), `monitoring/drift.py` at the repo root (`RollingQualityMonitor`, bounded `deque`), `RequestTimingMiddleware` (stashes a start timestamp; the route handler and every error handler in `errors.py` compute elapsed time and record on their own outcome — necessary because Starlette's exception handlers run *inside* `call_next`, so a middleware-computed elapsed value would not exist yet when a 422/500 handler runs). `GET /metrics`. `scripts/benchmark_serving.py` + `eval/report.py`'s `update_readme_section`. Done and tested (28 tests) and verified against a live smoke server's `/metrics` output.
